...
// Test 17: FromRequestID with arbitrary non-hex value produces a stable 32-hex
// trace_id (XOR-spread). Same input → same trace_id, deterministic across
// services (orchestator vs memory-engine).
func TestFromRequestID_ArbitraryString_Stable(t *testing.T) {
	spanSeed := make([]byte, 8)
	for i := range spanSeed {
		spanSeed[i] = byte(i + 3)
	}
	a, err := trace.FromRequestID("my-custom-request-id-xyz", deterministicReader(spanSeed))
	require.NoError(t, err)
	b, err := trace.FromRequestID("my-custom-request-id-xyz", deterministicReader(spanSeed))
	require.NoError(t, err)
	require.Len(t, a.TraceID, 32)
	require.Equal(t, a.TraceID, b.TraceID, "same X-Request-ID must produce same trace_id")
}
